<%@ page import="comum.Permissao; comum.Usuario" %>
...
<div class="fieldcontain ${hasErrors(bean: usuarioInstance, field: 'permissao', 'error')} required">
	<label for="permissao">
		<g:message code="usuario.permissao.label" default="Permissão" />
		<span class="required-indicator">*</span>
	</label>
	%{--<g:textField name="autor" required="" value="${livroInstance?.autor?.nome}"/>--}%
	<g:select name="permissao.id" value="${usuarioInstance?.id}" from="${comum.Permissao?.list()}" optionKey="id" optionValue="authority" />
